...
- **Report**: `name`, `type: report`, `owner` (wikilink do sujeito; o tipo sai da pasta da nota), `periodStart`, `periodEnd`, `created`, `topics`, `previousReport` (omitido no primeiro), `updated` (só depois de um `--update`).
