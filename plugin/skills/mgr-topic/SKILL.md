---
name: mgr-topic
description: Manage topics in the manager-assistant workspace - create, list, remove and archive. Use when the user says "cria um tópico", "adiciona tópico", "lista tópicos", "remove o tópico X", "arquiva o tópico X", "quais tópicos temos", "novo tópico", or runs /mgr-topic. Flags - "--list-topics" lists every Topic - *.md with status and the subjects that link to it; "--add" creates a new topic from the template and registers it in the subject(s) chosen; "--remove NAME" deletes the topic file and cleans every wikilink to it in Team/Forum/Person notes; "--archive NAME" renames the file to "Topic - archived - NAME.md", updates the H1 title and the frontmatter status to archived, and updates every subject wikilink to the new name. Other skills (mgr-setup, agents) that need to create a topic MUST delegate here with --add instead of creating the file themselves. Does not read trackers, messengers or meeting tools. Does not generate reports.
allowed-tools: Read Write Edit Glob Grep Bash(ls *) Bash(mv *) Bash(rm *) Bash(test *)
---

# mgr-topic

Single point of authority over `Topic - *.md` notes. Everything that creates, lists, removes or archives a topic goes through this skill; other skills invoke `/mgr-topic --add` instead of writing a topic file directly.
...
`WS` is `DATA_ROOT/workspaces/<activeWorkspace>`. Not found:
...
Read every `WS/Topic - *.md`, then every `WS/Team - *.md`, `WS/Forum - *.md`, `WS/Person - *.md` (`tracked: true`) to invert the relation.
...
| --- | --- | --- |
| <name> | <status> | <Team - X, Forum - Y> (or "—" when none) |
```
...
- Names in the argument that match `Team - *.md`, `Forum - *.md` or `Person - *.md` (`tracked: true`) are the chosen subjects.
...
Before creating anything, check `WS/Topic - <Name>.md`:
...
- File does not exist: read `templates/Topic.md`, fill it, and write `WS/Topic - <Name>.md`.
...
- Append a row to the `## Topics` table with `<Name>` | `[[Topic - <Name>]]` | first sentence of the description.
...
- Exact match on `Topic - <Name>.md` → target.
- No match: list existing topics (via list mode's inversion) and stop with `STATUS: BLOCKED`.

Before deleting, in `--interactive` mode, show the topic name, its subjects and how many reports reference it (from a Grep on `[[Topic - <Name>]]` inside `WS/Report - *.md`), and ask to confirm. In silent mode, proceed without confirmation but list the same facts in the summary and set `STATUS: WARN`.

Delete steps, in order:

1. `rm WS/Topic - <Name>.md`.
...
This skill does not edit report files on `--remove`; a report that referenced the topic keeps `[[Topic - <Name>]]` as a broken label. (On `--archive`, `link-keeper` does rewrite the wikilink in reports because it is a pointer update.)
...
If the topic filename already starts with `Topic - archived - `, `STATUS: WARN` and stop — already archived.

Steps, in order:

1. Read the topic note. Compute the new filename `Topic - archived - <Name>.md` and the new display name `archived - <Name>`.
...
3. `mv WS/Topic - <Name>.md WS/Topic - archived - <Name>.md`.
4. Invoke the `eis-manager-assistant:link-keeper` agent with the brief:
   ```
   operation: rename
   ws: <WS absolute path>
   old: [[Topic - <Name>]]
   new: [[Topic - archived - <Name>]]
   kind: topic
   ```
   The agent rewrites every occurrence of the old wikilink to the new one across `WS/*.md`, including subject frontmatter, `## Topics` tables, prose, and — by design — report files. Rewriting a wikilink target is a pointer update; the factual content of reports (`[Fn]`, farol, resumo) is untouched.
